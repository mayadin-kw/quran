(() => {
  ...
  class ReciterWordTimingProvider {
  ...
  async function prepareLiveTurn() {
    ...
    const turn = liveTurn = new window.QuranLiveRecitation(liveContext(), {
      ready: () => { if (/^TEACHER/.test(state.phase)) setFeedback("جاهز للتلاوة المباشرة"); },
      event: (event) => {
        if (!state || turn !== liveTurn) return;
  ...
  function correct() {
    if (![S.WAIT_VISIBLE, S.WAIT_HIDDEN, S.RECORD_VISIBLE, S.RECORD_HIDDEN, S.CHECK_VISIBLE, S.CHECK_HIDDEN].includes(state.phase)) return;
  ...
  function wrong(expectedIndex = null) {
    if (![S.WAIT_VISIBLE, S.WAIT_HIDDEN, S.CHECK_VISIBLE, S.CHECK_HIDDEN].includes(state.phase)) return;
    ...
    if (state.failures >= 3) { closeLiveTurn(); state.hidden = false; state.phase = S.SETUP; stage("جاري إعادة تحفيظ الآية"); setTimeout(startUnit, 700); }
  ...
  function consumeRecognition(result) {
    ...
    if (!hidden() && matched.length) { state.activeWordIndex = Math.max(...matched.map(Number)); render(); }
  ...
  async function record() {
    ...
    try {
      ...
      state.recordingStartedAt = performance.now(); recorder.start(); beginVad(); state.phase = hidden() ? S.RECORD_HIDDEN : S.RECORD_VISIBLE; render(); setFeedback("التحقق المباشر غير جاهز؛ سيُستخدم التحقق الاحتياطي لهذه المحاولة.");
    } catch { setFeedback("يلزم السماح باستخدام الميكروفون حتى يتم التحقق من التلاوة", true); }
  }
  function stopRecord() { if (liveRecording) { const wasHidden = hidden(); liveRecording = false; stopVad(); state.phase = wasHidden ? S.CHECK_HIDDEN : S.CHECK_VISIBLE; stage("تثبيت الكلمات…"); liveTurn?.end(); return; } if (recorder?.state === "recording") recorder.stop(); }
  ...
  function populateAyahs() { const count = app.ayahCounts[settings.surah]; for (const id of ["#memorizationFrom", "#memorizationTo"]) { const element = $(id), before = +element.value || 1; element.replaceChildren(...Array.from({ length: count }, (_, index) => new Option(`الآية ${nf.format(index + 1)}`, index + 1))); element.value = Math.min(before, count); } if (+$("#memorizationTo").value < +$("#memorizationFrom").value) $("#memorizationTo").value = $("#memorizationFrom").value; }
  ...
  $("#memorizationReciter").onchange = (event) => { settings.reciter = event.target.value; validSetup(); };
  $("#memorizationPreviewAudio").onclick = () => { const previewSurah = 1 + Math.floor(Math.random() * 114), previewAyah = 1 + Math.floor(Math.random() * app.ayahCounts[previewSurah]); audio.src = app.audioUrlFor(settings.reciter, `${previewSurah}:${previewAyah}`); audio.play().catch(() => $("#memorizationSetupMessage").textContent = "تعذر تشغيل المعاينة حالياً."); };
  $("#memorizationPermission").onclick = prepareMicrophone;
  if (debug) $("#memorizationSession").classList.add("debug");
  $("#startMemorization").onclick = start; $("#memorizationDemoCorrect").onclick = demoCorrect; $("#memorizationDemoError").onclick = () => wrong();
